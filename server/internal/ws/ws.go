...
func (h *Hub) AddClient(c *client) error {
	...
	go h.broadcastExcept(c.userID, Message{Type: "user_online", From: c.username})
	return nil
}
...
func (h *Hub) RemoveClient(userID int) {
	...
	if ok {
		go h.broadcastExcept(userID, Message{Type: "user_offline", From: client.username})
	}
}
...
func (c *client) readLoop() {
	...
	for {
		...
		if err != nil {
			c.trySend(Message{Type: "error", Body: "User not found: " + msg.To})
			continue
		}

		forwardMsg := Message{Type: "direct_message", From: c.username, Body: msg.Body}
		if !c.hub.SendDirect(recipientID, forwardMsg) {
			c.trySend(Message{Type: "error", Body: "User is not online: " + msg.To})
			continue
		}

		c.trySend(Message{Type: "message_ack", ID: msg.ID})
	}
}
